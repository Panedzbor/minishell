...
void assign_value_to_argv(t_tree_node *node, t_token *token)
{
    ...
    /* for (printf("assign_val_to_argv: "), count = 0; node->argv[count]; count++)
    {
        printf("%s ", node->argv[count]);
    }
    printf("\n"); */
}
...
void check_if_word_sequence(t_token **priora, t_token **priora_end)
{
    ...
    if (temp && *priora != temp)
        *priora_end = temp;
    // printf("check if word sequence: %s", (*priora)->value);
    //if (*priora_end)
    //    printf(" - %s", (*priora_end)->value);
    //printf("\n");
}
